...
@pytest.mark.django_db
@patch('ingestion.fetcher.requests.get')
def test_fetch_rate_payload_raises_after_retries(mock_get, settings):
    from ingestion.fetcher import IngestionFetcherError, fetch_rate_payload

    settings.INGESTION_RETRY_ATTEMPTS = 2
    mock_get.side_effect = requests.Timeout('upstream timed out')

    with pytest.raises(IngestionFetcherError, match='Failed to fetch'):
        fetch_rate_payload('https://example.com/rates/hsbc')
...
@pytest.mark.django_db
def test_ingest_rejects_wrong_bearer_token(sample_http_payload):
    client = APIClient()
    ...
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
    assert 'invalid bearer token' in str(response.json()).lower()
...
@pytest.mark.django_db
def test_load_parquet_file_respects_max_rows(monkeypatch):
    import pandas as pd
        ...
        [
            {
                ...
                'ingestion_ts': '2025-01-15T10:00:00Z',
                'source_url': 'https://example.com/rates/hsbc',
                'currency': 'USD',
                'raw_response_id': 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
            },
            {
                'provider': 'Barclays',
                'rate_type': 'savings_easy_access',
                ...
                'ingestion_ts': '2025-01-16T10:00:00Z',
                'source_url': 'https://example.com/rates/barclays',
                'currency': 'USD',
                'raw_response_id': 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb',
            },
        ]
    )
    monkeypatch.setattr(pd, 'read_parquet', lambda _: df)

    summary = load_parquet_file('ignored-path.parquet', batch_size=10, max_rows=1)

    assert summary == {'total_rows': 1, 'persisted_rates': 1, 'skipped_unparsed': 0}
    assert RawResponse.objects.count() == 1
