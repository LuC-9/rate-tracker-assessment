...
| `REQUIRE_ENV_VALIDATION` | No | `true` | When `true`, the app exits immediately if `DJANGO_SECRET_KEY`, `INGEST_API_TOKEN`, or `DB_PASSWORD` are missing |
| `NEXT_PUBLIC_API_URL` | Managed in Compose | `http://localhost:8000` | Frontend base URL injected into the Next.js container |

## Fail-fast behavior

Fail-fast validation is enabled in the provided `.env.example` (`REQUIRE_ENV_VALIDATION=true`), so copying `.env.example` to `.env` is sufficient for local runs that should stop early on missing required secrets. Set `REQUIRE_ENV_VALIDATION=false` only if you explicitly want to disable this startup validation.
...
Demo recordings are checked into the repository under `docs/demo/`:

- [Full walkthrough (webm)](docs/demo/walkthrough.webm)
- [Dashboard walkthrough (webm)](docs/demo/walkthrough-dashboard.webm)
- [Mobile walkthrough (webm)](docs/demo/walkthrough-mobile.webm)

The recordings cover the local review flow, including the dashboard at `localhost:3000`, chart interactions, API checks, and responsive behavior. Supporting screenshots and reproduction notes live in `docs/demo/README.md`.
